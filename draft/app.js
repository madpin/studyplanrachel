      ...
      <div class="task-category-content" id="content-${catIndex}">
        <ul class="task-list">
          ${taskCategory.items.map((item, itemIndex) => {
            const isCompleted = isTaskCompleted(date, catIndex, itemIndex);
            return `
              <li class="task-item ${isCompleted ? 'completed' : ''} ${taskCategory.isCatchUp ? 'catch-up' : ''}" data-sba="${item.isSBA ? 'true' : 'false'}" data-sba-key="${item.isSBA ? item.sbaKey : ''}">
                <input 
                  type="checkbox" 
                  class="task-checkbox" 
                  ${item.isSBA ? (isSBACompleted(dateStr, item.name) ? 'checked' : '') : (isCompleted ? 'checked' : '')}
                  onchange="${item.isSBA ? `handleSBAToggle('${dateStr}', this.parentElement.querySelector('.task-name').textContent.trim().split('\n')[0].trim())` : `handleTaskToggle(${catIndex}, ${itemIndex})`}"
                />
